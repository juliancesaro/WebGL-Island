...
function run(level) {
    ...
    for (let x = 0; x < width; x++) {
        for (let z = 0; z < depth; z++) {
            ...
            if(y == undefined || y1 == undefined || y2 == undefined || y3 == undefined){
                y = 0;
                y1 = 0;
                y2 = 0;
                y3 = 0;
    ...
    for (let i = 0; i < level.trees.length; i++) {
        ...
        let top = level.heightmap.height[level.trees[i][0] + level.trees[i][1] * width];
        let bottom = level.heightmap.height[level.trees[i][0] + level.trees[i][1] * width];
        let left = level.heightmap.height[level.trees[i][0] + level.trees[i][1] * width];
        let right = level.heightmap.height[level.trees[i][0] + level.trees[i][1] * width];

        if((level.trees[i][0] + level.trees[i][1] * width) - width >= 0) {
            top = level.heightmap.height[(level.trees[i][0] + level.trees[i][1] * width) - width];
        }
        if((level.trees[i][0] + level.trees[i][1] * width) + width <= level.heightmap.height.length - 1) {
            bottom = level.heightmap.height[(level.trees[i][0] + level.trees[i][1] * width) + width];
        }
        if((level.trees[i][0] + level.trees[i][1] * width) - 1 >= 0) {
            left = level.heightmap.height[(level.trees[i][0] + level.trees[i][1] * width) - 1];
        }
        if((level.trees[i][0] + level.trees[i][1] * width) + 1 <= level.heightmap.height.length - 1) {
            right = level.heightmap.height[(level.trees[i][0] + level.trees[i][1] * width) + 1];
        }

        height = level.heightmap.height[level.trees[i][0] + level.trees[i][1] * width];
        ...
        trees[i].branchPosition = [level.trees[i][0],height + 3,level.trees[i][1]];
    }
    ...
    let update = function(deltaTime) {
        ...
        let clickBound = [screen.width/3, screen.height/3,(screen.width/3)*2,(screen.height/3)*2,
            (screen.width/3)*2, screen.height/3, screen.width/3, (screen.height/3)*2];
        ...
        for(let i = 0; i < points.length; i = i + 18) {
            if(player.position[0] >= points[i] && player.position[0] <= points[i+6] &&
                player.position[2] >= points[i+2] && player.position[2] <= points[i+14]) {
                let sign = (points[i+3] - points[i]) * (player.position[2] - points[i+2]) -
                    (points[i+5] - points[i+2]) * (player.position[0] - points[i]);
                if(sign <= 0) {
                    let b00 = width - ((width - points[i+6]) + player.position[0])/
                        (width - (points[i] + (width - points[i+6])));
                    let b01 = (player.position[2] - points[i+2])/
                        (depth - (points[i+2] + (depth - points[i+5])));
                    let b02 = 1 - b00 - b01;
                    player.position[1] = ((b00*points[i+1]) + (b01*points[i+4]) + (b02*points[i+7]));
                }
                else if(sign >= 0) {
                    let b0 = (player.position[0] - points[i+12])/
                        (width - (points[i+12] + (width - points[i+15])));
                    let b1 = depth - ((depth - points[i+14]) + player.position[2])/
                        (depth - (points[i+11] + (depth - points[i+14])));
                    let b2 = 1 - b0 - b1;
                    player.position[1] = ((b0*points[i+16]) + (b1*points[i+10]) + (b2*points[i+13]));
                }
            }
        }
        ...
        if (inputManager.keyPressed["KeyA"]) {
            ...
            if (camera == true) {
                if((player.position[0] - player.trunkScale[0] - Math.sin(cameraRotation[1]) * 2 * deltaTime) >= 0 &&
                    (player.position[0] + player.trunkScale[0] - Math.sin(cameraRotation[1]) * 2 * deltaTime) <= width) {
                    player.position[0] -=
                        Math.sin(cameraRotation[1]) * 2 * deltaTime;
                }
                if((player.position[2] - player.trunkScale[2] - Math.cos(cameraRotation[1]) * 2 * deltaTime) >= 0 &&
                    (player.position[2] + player.trunkScale[2] - Math.cos(cameraRotation[1]) * 2 * deltaTime) <= depth) {
                    player.position[2] -=
        ...
        if (inputManager.keyPressed["KeyW"]) {
            if (camera == true) {
                if((player.position[0] - player.trunkScale[0] + Math.cos(-cameraRotation[1]) * 2 * deltaTime) >= 0 &&
                    (player.position[0] + player.trunkScale[0] + Math.cos(-cameraRotation[1]) * 2 * deltaTime) <= width) {
                    player.position[0] +=
                        Math.cos(-cameraRotation[1]) * 2 * deltaTime;
                }
                if((player.position[2] - player.trunkScale[2] + Math.sin(-cameraRotation[1]) * 2 * deltaTime) >= 0 &&
                    (player.position[2] + player.trunkScale[2] + Math.sin(-cameraRotation[1]) * 2 * deltaTime) <= depth) {
                    player.position[2] +=
                        Math.sin(-cameraRotation[1]) * 2 * deltaTime;
                }
            } else if(camera == false) {
        ...
        if (inputManager.mouseClickedPos[0] < (screen.width/3)*2 && inputManager.mouseClickedPos[0] > screen.width/3
        && inputManager.mouseClickedPos[1] < (screen.height/3)*2 && inputManager.mouseClickedPos[1] > screen.height/3) {
    ...
    let render = function() {
        ...
        if(camera == false) {
            ...
            // calculate the view matrix
            {
                ...
                glMatrix.vec3.set(cameraPosition, player.position[0],
                    player.position[1] + player.height, player.position[2]);

                glMatrix.vec3.rotateZ(fps, fps, cameraPosition, cameraRotation[2] * -1);
                glMatrix.vec3.rotateY(fps, fps,
                     player.position, cameraRotation[1] * -1);
            ...
            //lighting
            {
                ...
                gl.uniform3fv(shader["u_lightPosition"], new Float32Array(lightDirection));
            }
            ...
            // calculate the projection matrix
            {
                ...
                //glMatrix.mat4.perspective(projectionMatrix, fovy, aspect, near, far);
                glMatrix.mat4.ortho(projectionMatrix, -10, 10, -10, 10,
                    near, far);
                gl.uniformMatrix4fv(shader["u_projectionMatrix"], false, projectionMatrix);
            }
            ...
            // calculate the view matrix
            {
                glMatrix.vec3.set(cameraPosition, player.position[0] - cameraDistance,
                    player.position[1] + 5, player.position[2]);

                glMatrix.vec3.rotateZ(cameraPosition, cameraPosition,
                    player.position, cameraRotation[2]);
                glMatrix.vec3.rotateY(cameraPosition, cameraPosition,
                    player.position, cameraRotation[1]);

                gl.uniform3fv(shader["u_cameraPosition"], cameraPosition);

                const target = [player.position[0], player.position[1],
                    player.position[2]];
